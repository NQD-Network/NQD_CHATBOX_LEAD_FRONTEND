...
function detectTenant(): string {
  if (typeof window === 'undefined') {
    return 'nqd-chatbox'; // ✅ FIXED: Default for port 3001
  }
...
// 🔥 Check if user has access to tenant (with detailed logging)
...
export default function ProfilePage() {
  ...
  const [tenantId, setTenantId] = useState<string>('nqd-chatbox'); // ✅ FIXED: Default for 3001
      ...
      const attemptFetch = async (): Promise<void> => {
        try {
          ...
          if (!hasAccessToTenant(data, tenantId)) {
            const userTenants = getUserTenantNames(data);
  ...
  if (!mounted || !tokensProcessed || loading) {
    return (
      <div style={{ 
        ...
        <div style={{ marginBottom: '20px' }}>Loading user info...</div>
        <div style={{ fontSize: '12px', color: '#666' }}>
          Mounted: {mounted ? '✓' : '✗'} | 
          Tokens Processed: {tokensProcessed ? '✓' : '✗'} | 
          Loading: {loading ? '✓' : '✗'}
        </div>
      </div>
    );
